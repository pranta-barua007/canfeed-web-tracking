...
- **Analyze Duty First**: Before writing a test, understand the *responsibility* of the file. **Check for existing tests** to identify unnecessary mocks (e.g., mocking DB in a wrapper test). Only then proceed to test the crucial duty: Logic (algorithm) or Wrapper (delegation & I/O).
